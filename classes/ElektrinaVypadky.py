...
class ElektrinaVypadky:
    ...
    async def run(self) -> None:
        ...
        html_content = await self.session.get("https://www.hliniknadhronom.sk/mid/492460/ma0/all/.html")
    ...
    async def _get_articles(html_content: list) -> None:
        ...
        collumn = soup.find(class_="oznamy-new-columns-all-list-default oznamy-new-columns-all-list")
        # If site is unreachable
        if not collumn:
            logging.error("ElektrinaVypadky: Site is unreachable")
            return None
        articles = collumn.find_all("div", class_="short-text-envelope-default short-text-envelope")
        return articles[:ELEKTRINA_MAX_ARTICLES]

    async def _process_articles(self, articles: list, elektrinavypadky_cache: list) -> None:
        above_limit = False
        for article in articles:
            description = article.find("div").text.lower()
            title = article.find("a")["aria-label"]

            if not ("elektriny" in description or "elektriny" in title.lower() or "odstávka vody" in title.lower()):
                continue
            ...
            elektrinavypadky_cache = [elektrinavypadky_cache[-1]] + elektrinavypadky_cache[:-1]
